<script lang="ts">
  ...
  const COND_WAIT: Record<string, string> = {
    ...
    reserve: 'Les batteries se rechargent d’abord',
    residu: 'Le dernier essai a acheté du courant — on attend le retour du soleil',
  ...
  const COND_HUMAN: Record<string, string> = {
    ...
    reserve: 'Assez d’énergie pour finir la chauffe',
    residu: 'Surplus revenu depuis l’échec',
