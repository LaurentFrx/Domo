...
<script lang="ts">
  ...
  /**
   * `page` : la mise en page de l'accueil (grille iPad/desktop d'origine).
   * `column` : une pile, pour une colonne étroite du tableau de bord /bureau —
   * les grilles internes se dérouleraient à l'horizontale sinon, les media
   * queries lisant la largeur du VIEWPORT et non celle de la colonne.
   */
...
<style>
  ...
  /* En colonne (/bureau) : on annule les deux grilles ci-dessus — la largeur
     disponible est celle de la colonne, pas celle de l'écran. */
  .home-grid.home-col {
    display: flex;
    /* `align-items: start` vient de la grille iPad ci-dessus : en pile, il
       empêcherait les cartes de prendre la largeur de la colonne — le Sankey,
       qui se dimensionne sur sa largeur, se réduisait à rien. */
    align-items: stretch;
  }
