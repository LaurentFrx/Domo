<script lang="ts">
  ...
  /**
   * `page` : la mise en page de /climat (grille commune à deux colonnes dès
   * l'iPad). `column` : une pile, pour la colonne « Climat » de /bureau — les
   * `pad:` lisent la largeur du VIEWPORT, jamais celle de la colonne.
   */
...
<div class="flex flex-col gap-6 py-4">
  ...
  <div
    class={column
      ? 'flex flex-col gap-3'
      : 'pad:grid pad:grid-cols-2 pad:items-stretch pad:gap-3 contents'}
  >
    <!-- ═══ Daikin Séjour + Salle de bain — cartes instrument ═══ -->
    <!-- iPhone portrait : 1 carte/ligne (confort) ; écran large : 2 colonnes. -->
    <section class={column ? 'flex flex-col gap-3' : 'pad:contents'}>
      ...
      <div
        class={column
          ...
          : 'pad:contents grid grid-cols-1 gap-3 sm:grid-cols-2'}
      >
        <!-- ╔═══════════════════════════════════════════════════════════════════╗
    ...
    <section class={column ? 'flex flex-col gap-3' : 'pad:contents flex flex-col gap-3'}>
      {#if azOffline}
      ...
      <div
        class={column
          ? 'grid grid-cols-1 gap-3'
          : 'pad:contents grid grid-cols-1 gap-3 sm:grid-cols-3'}
      >
    ...
    {#if thermoSensors.length > 0}
      <section class={column ? 'flex flex-col gap-3' : 'pad:contents'}>
        <div class="thermo-grid grid grid-cols-3 gap-2" class:thermo-row={column}>
  ...
  <section
    class="flex flex-col gap-2.5 rounded-[var(--radius-2xl)] border p-3"
    class:pad:flex-row={!column}
    class:pad:gap-0={!column}
    style="background: var(--color-card); border-color: var(--color-border);"
  >
    <!-- ── Maintenant (compact : icône + temp à gauche, lieu à droite) ── -->
    <div class="flex flex-col gap-2" class:pad:flex-1={!column} class:pad:pr-6={!column}>
    ...
    <div class:pad:flex-1={!column} class:pad:pl-6={!column}>
      <span
...
<style>
  ...
  /* En colonne (/bureau), la cellule verticale de la grille commune n'existe
     plus : les trois thermomètres reprennent leur ligne, et rendent 120 px. */
  .thermo-grid.thermo-row {
