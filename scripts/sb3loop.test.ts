...
// ─── DESCENTE D'EXCÈS — le recyclage SB3→Max AC est retiré immédiatement ──
// Contexte (02/08) : bouilloire 1 404 W × 1 min → montée d'urgence, puis
// ~12 min de descente progressive pendant lesquelles ~300 Wh passaient des
// SB3 (46 %) vers la Max AC (61 %) à travers deux conversions. La branche
// retire l'excès EN UNE ÉCRITURE, sans jamais demander à la Max de fournir.

test('DESCENTE D’EXCÈS : la Max AC absorbe à compteur nul → l’excès est retiré en UNE écriture', () => {
  const d = decide(
    inp({
      em50: { ok: true, gridW: 0 },
      maxac: { ok: true, socPct: 60, acNetW: -1400, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60), sb3OutW: 1500 }
    }),
    cfg,
    st({ lastCmdW: 1500 })
  );
  assert.equal(d.mode, 'allocate');
  assert.ok(d.reason.includes("DESCENTE D'EXCÈS"), d.reason);
  assert.equal(d.writeW, 100); // 1500 − 1400 (plancher part ≈ 43 W, non mordant)
});

test('DESCENTE D’EXCÈS : la correction n’est PAS inscrite « en vol » (la Max l’absorbe, le compteur ne la verra jamais)', () => {
  const d = decide(
    inp({
      maxac: { ok: true, socPct: 60, acNetW: -1400, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60), sb3OutW: 1500 }
  ...
  assert.ok(d.reason.includes("DESCENTE D'EXCÈS"), d.reason);
  assert.equal(d.enVol.length, 0); // un {dW:−1400} en vol aurait fabriqué un faux soutirage au tick suivant
});

test('DESCENTE D’EXCÈS : scénario bouilloire du 02/08 — la consigne rentre en DEUX ticks (~50 s), plus 12 min', () => {
  // t0 : appel de 1 404 W → montée d'urgence (règle 1, comportement inchangé).
  const d1 = decide(
    inp({
      em50: { ok: true, gridW: 1404 },
      maxac: { ok: true, socPct: 60, acNetW: 2000, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60), sb3OutW: 150 }
    }),
    cfg,
    st({ lastCmdW: 150 })
  );
  assert.equal(d1.writeW, 1554);
  // t0+25 s : l'appareil est éteint, la Max AC gobe l'excédent à compteur nul.
  const d2 = decide(
    inp({
      now: NOW + 25_000,
      em50: { ok: true, gridW: -20 },
      maxac: { ok: true, socPct: 60, acNetW: -1300, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60) }
    }),
    cfg,
    st({ lastCmdW: 1554, lastWriteTs: NOW, enVol: d1.enVol })
  );
  assert.ok(d2.reason.includes("DESCENTE D'EXCÈS"), d2.reason);
  assert.equal(d2.writeW, 254); // 1554 − 1300 : tout l'excès, en une écriture
});

test('DESCENTE D’EXCÈS : garde de VISIBILITÉ — pas de re-descente avant que la précédente paraisse dans acNet', () => {
  const d = decide(
    inp({
      now: NOW + 10_000, // 10 s < enVolS(20 s) après la dernière écriture
      maxac: { ok: true, socPct: 60, acNetW: -1400, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60) }
    }),
    cfg,
    st({ lastCmdW: 1500, lastWriteTs: NOW })
  );
  assert.equal(d.writeW, null); // l'excès affiché peut être l'ancien monde : on attend UN tick
});

test('DESCENTE D’EXCÈS : garde ffHold — l’excédent d’un pré-armement cumulus est VOULU, on n’y touche pas', () => {
  // Pré-armement écrit il y a 25 s (visible ✓, settling actif) : SEULE la
  // descente d'excès pourrait écrire — le hold doit la tenir.
  const d = decide(
    inp({
      now: NOW,
      maxac: { ok: true, socPct: 60, acNetW: -1800, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60) }
    }),
    cfg,
    st({ lastCmdW: 2000, lastWriteTs: NOW - 25_000, ffHoldUntilTs: NOW + 5_000 })
  );
  assert.equal(d.writeW, null);
  assert.ok(!d.reason.includes('DESCENTE'), d.reason);
});

test('DESCENTE D’EXCÈS : garde Max AC RETARDATAIRE — on ne défait pas le détour de la règle 0', () => {
  // Max AC à 15 % contre SB3 90 % : sa charge est le TRAVAIL du rééquilibrage.
  const d = decide(
    inp({
      maxac: { ok: true, socPct: 15, acNetW: -600, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(90), sb3OutW: 800, sb3PvW: 1600 }
    }),
    cfg,
    st({ lastCmdW: 800 })
  );
  assert.ok(!d.reason.includes('DESCENTE'), d.reason);
  assert.ok(d.reason.includes('RÉÉQUILIBRAGE'), d.reason); // le détour continue, lui
});

test('DESCENTE D’EXCÈS : Max AC qui DÉBITE = aucun excès, la branche est inerte', () => {
  const d = decide(
    inp({
      maxac: { ok: true, socPct: 60, acNetW: 500, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60), sb3OutW: 1500 }
...
test('DESCENTE D’EXCÈS : la RÈGLE 1 prime — un soutirage mesuré passe avant tout', () => {
  const d = decide(
    inp({
      em50: { ok: true, gridW: 400 },
      maxac: { ok: true, socPct: 60, acNetW: -300, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60), sb3OutW: 1500 }
    }),
    cfg,
    st({ lastCmdW: 1500 })
  );
  assert.ok(d.reason.includes('SOUTIRAGE'), d.reason);
  assert.equal(d.writeW, 1900);
});

test('DESCENTE D’EXCÈS : un excès sous la bande morte ne déclenche rien (résolution, pas zèle)', () => {
  const d = decide(
    inp({
      maxac: { ok: true, socPct: 60, acNetW: -80, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60), sb3OutW: 300 }
    }),
    cfg,
    st({ lastCmdW: 300 })
  );
...
test('DESCENTE D’EXCÈS : le PLANCHER prorata tient — on retire l’excès, jamais la part (règle 2)', () => {
  // sb3Out cloud (2400) > consigne (1200) : la borne « consigne − excès »
  // descendrait à 200, sous la part légitime → le plancher la retient.
  const d = decide(
    inp({
      maxac: { ok: true, socPct: 60, acNetW: -1000, ratedEnergyWh: 7200 },
      cloud: { ...inp().cloud, sb3Packs: packs(60), sb3OutW: 2400 }
    }),
    cfg,
    st({ lastCmdW: 1200 })
  );
  assert.ok(d.reason.includes("DESCENTE D'EXCÈS"), d.reason);
  assert.ok(
    (d.writeW ?? 0) >= 590 && (d.writeW ?? 0) <= 610,
    `plancher ≈ 598 W (obtenu ${d.writeW})`
  );
});
