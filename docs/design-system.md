...
- **La tuile montre l'objet EN SERVICE** (refonte du 19/09/2026) : chaque ligne de LED est dessinée comme un ruban lumineux (un trait par ligne, dans l'ordre physique — une ligne éteinte reste un trait sombre), dans la couleur de sa LUMIÈRE. **Aucun halo**, ni sur la carte ni autour des rubans (retirés le 19/09/2026 à la demande de Laurent) : la lumière, ce sont les rubans, le niveau se lit à leur intensité. Le niveau se lit à l'intensité ; le pourcentage ne s'écrit que pendant le glissé. Plus de carte peinte en plein : l'aperçu direct y arrivait blanchi (cf. ci-dessous) puis voilé de bleu nuit sous le texte — gris à gauche, olive à droite, « très moche » (Laurent).
- **Texte minimal** : le lieu (« Terrasse »), l'interrupteur, le bouton Réglages. Ni surtitre, ni nom d'appareil, ni nom d'effet, ni gros pourcentage : ce que la lumière montre d'elle-même ne s'écrit pas. Seul reste ce qui ne se voit pas (état du mode Musique, badge Hors ligne / Démo). Le texte, en haut, est sur le verre nu : aucun voile n'est nécessaire.
- **Couleur de la LUMIÈRE, pas addition sRGB** : une LED RGBW mélange la teinte et la puce blanche 4000K. Les additionner en sRGB sature (ambre + blanc = blanc pur, la chaleur disparaît). `lightColor()` (`$lib/wled/preview-model`) les mélange en **linéaire** puis ramène à pleine luminance : la température de couleur survit (ambre + blanc = `rgb(255 191 142)`, blanc seul = `rgb(255 223 191)`). Ne pas la « raviver » (`vividTint`) : ce serait tordre justement la température.
- **⚠️ L'aperçu direct de WLED ment dès que le blanc est allumé** : le firmware y AJOUTE le canal blanc à chaque canal (`qadd8(w, c)`, live view). Il ne sert donc qu'aux lignes sans blanc (`seg.white === 0`) ; sinon, couleur calculée. La feuille (`WledSheet`) l'utilise encore tel quel.
- **Thème clair** : une ligne blanche sur verre clair disparaît — la ligne allumée est posée dans un profilé sombre de 1 px (le rail alu du vrai ruban). Inutile en sombre, où il éteindrait le halo.
...
- **Les feuilles descendent du HAUT** (04/08/2026) — posées en bas, leurs premiers réglages tombaient hors de portée sur iPhone. Conséquences à ne pas oublier en les touchant : zone sûre du **haut** (encoche / Dynamic Island) **et des côtés** (paysage iPhone — rien ne verrouille l'orientation, iOS ignore le membre `orientation` du manifest), arrondis en bas, animation `translateY(-100%)`, hauteur en **dvh** (le vh de Safari inclut la barre d'URL), et poignée iOS sur le bord **libre** (en bas, fixe sous le scroller). ⚠️ Deux implémentations à garder d'accord : `BottomSheet` (verre Yeldra, 3 écrans) et `MenuSheet` (surface iOS, menu ☰) — la seconde ne réutilise PAS la première. Le nom `BottomSheet` est resté malgré l'ancrage haut.
- **Anatomie d'une feuille : le panneau ne défile JAMAIS lui-même.** Panneau = cadre (`flex column`, `overflow: hidden`, porte les zones sûres) ; seul le **corps** défile (`flex: 1 1 auto; min-height: 0; overflow-y: auto`). Un overflow clippe au **padding box** : un panneau-scroller laisse le contenu défilé visible dans le padding de zone sûre, sous la Dynamic Island — et emporte l'en-tête (croix comprise) au premier défilement. Complément clavier : `aria-modal` ne rend PAS le fond inerte — piéger Tab dans la feuille, fermer sur `afterNavigate` (retour matériel), rendre le focus à l'ouvreur.
- **⚠️ PIÈGE MAJEUR — aucune surface `position: fixed` montée DANS une page du Pager.** Le rail (`.pager-rail`) porte `will-change: transform` en permanence : un ancêtre transformé devient le référentiel des `position: fixed` de ses descendants — la modale s'ancre à la PAGE (longue), pas au viewport, et s'ouvre hors écran ; les cellules voisines ont en plus `overflow: hidden`. C'est le bug qui a rendu la feuille terrasse invisible (05/08/2026). Deux remèdes selon le cas : composant autonome → **montage dans `+layout.svelte`** + état module-level (pattern `menu-state` / `wled-sheet-state`) ; modale tissée dans l'état local d'une page → **action `portal`** (`$utils/portal`, ancre `#layout-modal-root`). La cible du portail doit rester dans l'arbre monté par Svelte (délégation d'événements), jamais `document.body`.
